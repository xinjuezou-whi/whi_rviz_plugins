...
******************************************************************/
#pragma once
#include <whi_interfaces/msg/whi_line_chart2_d.hpp>
...
namespace whi_rviz_plugins
{
	...
	public:
		...
		TimeData(double Value, const std::string& Unit,
			double Sec = rclcpp::Clock().now().seconds(), double BaseTime = rclcpp::Clock().now().seconds())
	...
	public:
		void add(const std::string& Key, double Value, const std::string& Unit, double Sec = rclcpp::Clock().now().seconds());
	...
	private:
		...
		void subCallbackData(const whi_interfaces::msg::WhiLineChart2D::SharedPtr Msg);
		void plot();
