...
******************************************************************/
#include "whi_rviz_plugins/display_tf.h"
#include "whi_rviz_plugins/frame_info.h"
...
namespace whi_rviz_plugins
{
	...
	FrameInfo *TFDisplay::createFrame(const std::string &frame)
	{
		...
		info->euler_property_ = new VectorProperty(
			"Euler", Ogre::Vector3::ZERO,
			"Euler of this frame, in the current Fixed Frame.  (Not editable)",
			info->enabled_property_);
		info->euler_property_->setReadOnly(true);
		...
		info->rel_euler_property_ = new VectorProperty(
			"Relative Euler", Ogre::Vector3::ZERO,
			"Euler of this frame, relative to it's parent frame.  (Not editable)",
			info->enabled_property_);
		info->rel_euler_property_->setReadOnly(true);
		...
		const std::string &message) const
	{
		RVIZ_COMMON_LOG_DEBUG_STREAM(
			"Error transforming from frame '" << parent_frame.c_str() << "' to frame '" << child_frame.c_str() << "' with fixed frame '" << qPrintable(fixed_frame_) << "': " << message);
	}
		...
		std::shared_ptr<tf2::BufferCore> tf_buffer) const
	{
		...
		tf2::Quaternion q(transform.transform.rotation.x, transform.transform.rotation.y, transform.transform.rotation.z,
...
PLUGINLIB_EXPORT_CLASS(whi_rviz_plugins::TFDisplay, rviz_common::Display)
