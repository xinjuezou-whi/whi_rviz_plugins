...
namespace whi_rviz_plugins
{
    DisplayBat::DisplayBat()
    {
        ...
        color_property_ = new rviz_common::properties::ColorProperty("Color", QColor(138, 226, 52),
        ...
        size_property_ = new rviz_common::properties::FloatProperty("Size", 1.0,
            "Character size of battery info text.",
            this, SLOT(updateSize()));

        offsets_property_ = new rviz_common::properties::VectorProperty("Offsets", Ogre::Vector3::ZERO,
            "Offsets to frame",
            this, SLOT(updateOffsets()));

        orientation_property_ = new rviz_common::properties::VectorProperty("Orientation",
            Ogre::Vector3(float(0.0), float(90.0), float(0.0)), "Orientation of battery symbol",
    ...
    void DisplayBat::processMessage(const whi_interfaces::msg::WhiBattery::ConstSharedPtr Msg)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("DisplayBat"), "error transforming from frame '%s' to frame '%s'",
        ...
        float alpha = alpha_property_->getFloat();
        Ogre::ColourValue color = Msg->state == whi_interfaces::msg::WhiBattery::STA_NEED_CHARGING ?
            *color_red_ : color_property_->getOgreColor();
        visual->setColor(color.r, color.g, color.b, alpha);
        visual->createBatteryShape(Msg->soc / 100.0, Msg->state == whi_interfaces::msg::WhiBattery::STA_NEED_CHARGING ?
...
PLUGINLIB_EXPORT_CLASS(whi_rviz_plugins::DisplayBat, rviz_common::Display)
