...
namespace whi_rviz_plugins
{
    ...
    void TimeSeriesData::add(const std::string& Key, double Value, const std::string& Unit,
        double Sec/* = rclcpp::Clock().now().seconds()*/)
    {
    ...
    void LineChart2DPanel::setDataTopic(const std::string& Topic)
    {
		if (!Topic.empty())
		{
            sub_data_ = node_handle_->create_subscription<whi_interfaces::msg::WhiLineChart2D>(
                Topic, 10, std::bind(&LineChart2DPanel::subCallbackData, this, std::placeholders::_1));
		}
    }
    ...
    {
        auto channel = ui_->comboBox_channel->currentText().toStdString();
        chart_map_[channel].setBaseTime(rclcpp::Clock().now().seconds());

        plot();
    }
    
    void LineChart2DPanel::subCallbackData(const whi_interfaces::msg::WhiLineChart2D::SharedPtr Msg)
    {
        // check if the channel changes
        bool chChanged = false;
        if (ui_->comboBox_channel->count() != Msg->array.size())
        {
            chChanged = true;
        }
        else
        {
            ...
            {
                if (ui_->comboBox_channel->itemText(i).toStdString() != Msg->array[i].name)
                {
                    chChanged = true;
                    break;
                }
        ...
        for (int i = 0; i < Msg->array.size(); ++i)
        {
            if (chChanged)
            {
                ui_->comboBox_channel->addItem(Msg->array[i].name.c_str());
            }

            if (auto search = chart_map_.find(Msg->array[i].name); search == chart_map_.end())
            {
                chart_map_[Msg->array[i].name] = TimeSeriesData(max_data_length_);
            }
            for (int j = 0; j < std::min(Msg->array[i].data.size(), Msg->array[i].items_name.size()); ++j)
            {
                chart_map_[Msg->array[i].name].add(Msg->array[i].items_name[j],
                    Msg->array[i].data[j], Msg->array[i].items_unit[j], rclcpp::Time(Msg->header.stamp).seconds());
            }
        }
        ...
        else
        {
            ...
            if (ui_->comboBox_item->count() != Msg->array[index].items_name.size())
            {
                namesChanged = true;
            }
            else
