...
namespace whi_rviz_plugins
{
	WaypointsPanel::WaypointsPanel(VisualizeWaypoints FuncWaypoints, VisualizeEta FuncEta,
		...
		, node_handle_(std::make_shared<rclcpp::Node>("WaypointsPanel"))
	{
		// set up the GUI
		ui_->setupUi(this);

		// WHI logo
		std::string path = ament_index_cpp::get_package_share_directory("whi_rviz_plugins");
		QImage logo;
		if (logo.load(QString(path.c_str()) + "/icons/classes/whi_logo.png"))
		{
		...
		// plugins
		loadPlugin(path + "/config/config.yaml", ui_->comboBox_ns->currentText().toStdString());
	...
	void WaypointsPanel::useStampedVel(bool Flag)
	{
		for (auto& it : goals_map_)
		{
			if (it.second)
			{
				it.second->setUseStampedVel(Flag);
			}
		}
	}
	...
	void WaypointsPanel::updateWaypoint(int Index, const geometry_msgs::msg::Pose& Pose)
	{
	...
	void WaypointsPanel::setMotionStateTopic(const std::string& Topic)
	{
		if (!Topic.empty())
		{
			sub_motion_state_ = node_handle_->create_subscription<whi_interfaces::msg::WhiMotionState>(
				Topic, 10, std::bind(&WaypointsPanel::subCallbackMotionState, this, std::placeholders::_1));
		}
	}
	...
	void WaypointsPanel::setSwEstopTopic(const std::string& Topic)
	{
		if (!Topic.empty())
		{
			sub_sw_estop_ = node_handle_->create_subscription<std_msgs::msg::Bool>(
				Topic, 10, std::bind(&WaypointsPanel::subCallbackSwEstop, this, std::placeholders::_1));
		}
	}
	...
	void WaypointsPanel::setRcStateTopic(const std::string& Topic)
	{
		if (!Topic.empty())
		{
			sub_rc_state_ = node_handle_->create_subscription<whi_interfaces::msg::WhiRcState>(
				Topic, 10, std::bind(&WaypointsPanel::subCallbackRcState, this, std::placeholders::_1));
		}
	}
	...
	void WaypointsPanel::fillWaypoint(int RowIndex, bool WithCurrent/* = false*/, const std::vector<double>* Point/* = nullptr*/)
	{
		...
		if (WithCurrent)
		{
			geometry_msgs::msg::Pose currentPose =
				goals_map_[ui_->comboBox_ns->currentText().toStdString()]->getCurrentPose();
	...
	bool WaypointsPanel::loadWaypoints(std::string File)
	{
		try
		{
			...
			int ret = QMessageBox::Yes;
			std::string ns = ui_->comboBox_ns->currentText().toStdString();
			geometry_msgs::msg::Pose origin = goals_map_[ns]->getMapOrigin();
	...
	std::vector<geometry_msgs::msg::PoseStamped> WaypointsPanel::convertToAbsolute(
		const std::vector<WaypointPack>& WaypointPacks) const
	{
		std::vector<geometry_msgs::msg::PoseStamped> absolutes;
	...
	double WaypointsPanel::getYawFromPose(const geometry_msgs::msg::Pose& Pose) const
	{
	...
	bool WaypointsPanel::loadPlugin(const std::string& Config, const std::string& Namespace)
	{
    	...
    	{
        	RCLCPP_ERROR_STREAM(node_handle_->get_logger(), "\033[1;31m" << "failed to load config file " <<
				Config << "\033[0m");
        	return false;
    	}
	}
	...
	bool WaypointsPanel::createTaskPlugin(const YAML::Node& Node, const std::string& Namespace)
	{
        try
        {
			if (!plugins_map_[task_plugin_name_])
			{
				...
				plugins_map_[task_plugin_name_] = plugin_loader_->createSharedInstance(task_plugin_name_);
				plugins_map_[task_plugin_name_]->initialize(Node, Namespace);
			}
            
            RCLCPP_INFO_STREAM(node_handle_->get_logger(), "created plugin " << task_plugin_name_);

			return true;
        }
        catch (const pluginlib::PluginlibException& ex)
        {
            RCLCPP_FATAL_STREAM(node_handle_->get_logger(), "\033[1;31m" << "failed to create the " << task_plugin_name_ <<
	...
	void WaypointsPanel::subCallbackMotionState(const whi_interfaces::msg::WhiMotionState::SharedPtr Msg)
    {
    ...
    void WaypointsPanel::subCallbackSwEstop(const std_msgs::msg::Bool::SharedPtr Msg)
    {
