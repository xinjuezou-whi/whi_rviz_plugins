...
namespace whi_rviz_plugins
{
	BatteryVisual::BatteryVisual(Ogre::SceneManager* SceneManager, Ogre::SceneNode* ParentNode)
	{
		...
		battery_info_.reset(new rviz_rendering::MovableText("?\%"));
	...
	void BatteryVisual::setMessage(const whi_interfaces::msg::WhiBattery::ConstSharedPtr Msg)
	{
		int h = 0;
		int v = 0;
		battery_info_->setTextAlignment(rviz_rendering::MovableText::H_CENTER, rviz_rendering::MovableText::V_CENTER);
		rviz_common::properties::StringProperty text("text", (std::to_string(Msg->soc) + "\%").c_str());
	...
	void BatteryVisual::createBatteryShape(double PowerRatio, const Ogre::ColourValue& Color)
	{
		...
		battery_shape_[0].reset(new rviz_rendering::Shape(rviz_rendering::Shape::Cylinder, scene_manager_));
		...
		battery_shape_[1].reset(new rviz_rendering::Shape(rviz_rendering::Shape::Cylinder, scene_manager_));
		...
		battery_shape_[2].reset(new rviz_rendering::Shape(rviz_rendering::Shape::Cylinder, scene_manager_));
		...
		battery_shape_[3].reset(new rviz_rendering::Shape(rviz_rendering::Shape::Cylinder, scene_manager_));
